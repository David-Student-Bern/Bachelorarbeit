...
def wait_times_energies(Okamoto_df, short_KIC):
    ...
    for KIC in short_KIC:
        ...
        for i in range(np.size(Okamoto_KIC.values[:,0])-1):
            Okamoto_wait_times.append(Okamoto_KIC.values[i+1,3]-Okamoto_KIC.values[i,3])
            Okamoto_energies1.append(Okamoto_KIC.values[i,6])
            Okamoto_energies2.append(Okamoto_KIC.values[i+1,6])
            Okamoto_flare_length.append(Okamoto_KIC.values[i,2]-Okamoto_KIC.values[i,1])
            Okamoto_energies.append(Okamoto_KIC.values[i,6])
        if np.size(Okamoto_KIC.values[:,2])>0:
            Okamoto_flare_length.append(Okamoto_KIC.values[-1,2]-Okamoto_KIC.values[-1,1])
            Okamoto_energies.append(Okamoto_KIC.values[-1,6])
...
# AFD
# Okamoto_df = pd.read_csv("C:/Users/david/Documents/David/Unibe/Bachelorarbeit/AFD/Kepler_Plots_all/final-flares_all.csv")
...
def plot_histogram(data, plot_filter, bins_set, range_array):
    ...
    ax.set_title("Flatwrm: Histogram for wait times of flares")
    ax.grid(True)
    plt.show()
...
def plot_energies(data, energies, plot_filter):
    ...
    # Put a legend to the right of the current axis
    ax.legend(loc='center left', bbox_to_anchor=(1, 0.5))
    ax.set_title("Flatwrm: Energies and Wait times")
    plt.show()
...
def plot_flare_length(data, energies, plot_filter):
    ...
    # Put a legend to the right of the current axis
    ax.legend(loc='center left', bbox_to_anchor=(1, 0.5))
    ax.set_title("Flatwrm: Energies and Flare duration")
    ax.grid(True)
    plt.show()
...
def plot_prot(data, energies):
    ...
    ax.set_ylabel('flare energy [ergs]')
    ax.legend()
    ax.set_title("Flatwrm: Energies and rotational Period")
    ax.grid(True)
    plt.show()
